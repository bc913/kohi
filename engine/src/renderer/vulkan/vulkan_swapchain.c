...
// Fences are used to sync operations (happening on gpu) between actual app and GPU
// Semaphore are used to sync GPU operations with other GPU operations
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {  // Not a failure
...
// Returning the image with present_image_index back to the swapchain for presentation
// It is queued op so we need semaphore for that
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    // We're gonna be using triple buffer if we can
    // We can have two frames being rendered to as we are drawing a third frame
    ...
    // Use the first one if available not found
    if (!found) {
        swapchain->image_format = context->device.swapchain_support.formats[0];
    }

    VkPresentModeKHR present_mode = VK_PRESENT_MODE_FIFO_KHR;  // it always renders frames in order within the queue
    for (u32 i = 0; i < context->device.swapchain_support.present_mode_count; ++i) {
        ...
        if (mode == VK_PRESENT_MODE_MAILBOX_KHR) {  // Use most current image available and throw out older ones
            present_mode = mode;
            break;
        }
    }
    ...
    } else {  // Use the same queue we don't share the images between queues
        swapchain_create_info.imageSharingMode = VK_SHARING_MODE_EXCLUSIVE;
        swapchain_create_info.queueFamilyIndexCount = 0;
        swapchain_create_info.pQueueFamilyIndices = 0;
    }

    // Transform specs of the images (i.e. rotation, clipped, portrait mode)
    ...
    // Creates the images and its image views
    ...
    // Depth buffer is an image that gets written to with depth information
    ...
    vulkan_image_create(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,  // Use gpu memory
        TRUE,
