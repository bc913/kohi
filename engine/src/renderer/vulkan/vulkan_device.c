...
typedef struct vulkan_physical_device_requirements {
    // device has queues which takes commands to be executed in that queue
    // queue types
    b8 graphics;  // draw calls
    b8 present;   // presentation to the screen
    b8 compute;   // compute shaders
    b8 transfer;  //  transitioning data from one format to another
    // darray
...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    // Do NOT use queues seperately if they have present support and transfer
...
b8 physical_device_meets_requirements(
    ...
    // Mark all queues to -1 (not supported) initially
    ...
    // Each family has some number of queues and all of the queues in the family
    // can support one or multiple operations i.e. Graphics, Present and/or Transfer
    ...
    if (
        ...
        if (requirements->device_extension_names) {
            u32 available_extension_count = 0;
            VkExtensionProperties* available_extensions = 0;
            // Obtain the count first
            VK_CHECK(vkEnumerateDeviceExtensionProperties(
                device,
                0,
                &available_extension_count,
                0));
            // If not zero, obtain them
            if (available_extension_count != 0) {
                available_extensions = kallocate(sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
                    device,
                ...
                // Iterate over required extensions and check if they exist in available extensions
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    // Free already allocated ones if not found
                    if (!found) {
