...
b8 vulkan_graphics_pipeline_create(
    ...
    // TODO: VIEWPORT AND SCISSORS can be multiple and configurable
    // Viewport state
    ...
    // It is not like opengl. You can't change the view to wireframe on the go.
    // You have to create a new pipeline
    ...
    // How to compare two depths. Determines what gets clipped.
    depth_stencil.depthCompareOp = VK_COMPARE_OP_LESS;
    depth_stencil.depthBoundsTestEnable = VK_FALSE;
    depth_stencil.stencilTestEnable = VK_FALSE;

    // The way that how colors are written to the image
    ...
    // Dynamic state
    // Pipelines are immutable but there are some stuff can be changed during runtime
    // This is achieved by dynamic state
    ...
    // Vertex input
    // Holds several types of info i.e. Coordinates, color, texture mapping coordinates
    VkVertexInputBindingDescription binding_description;
    binding_description.binding = 0;                              // Binding index
    binding_description.stride = sizeof(vertex_3d);               // in bytes
    binding_description.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;  // Move to next data entry for each vertex.

    // TODO: Should be configurable
    // Attributes
    // layout(location=0) in glsl is an attribute
    ...
    // i.e. You can create one pipeline for wireframe one for non-wireframe
