...
/**
 * Actually generates framebuffer. Each time window is resized, you also have to regenerate the frame buffers
 * because the images in the swapchain requires update in the size and framenbuffers are attached to them.
 */
...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name) {
    ...
    darray_push(required_extensions, &VK_KHR_SURFACE_EXTENSION_NAME);  // Generic surface extension (surface is needed to draw sth on screen)
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend) {
    ...
    // Destroy created objects in reversed order so destroy debugger before instance
...
/**
 * We need command buffer for each one of swapchain images
 *
 */
void create_command_buffers(renderer_backend* backend) {
...
/**
 * We need a framebuffer for each swapchain image
 */
void regenerate_framebuffers(renderer_backend* backend, vulkan_swapchain* swapchain, vulkan_renderpass* renderpass) {
    for (u32 i = 0; i < swapchain->image_count; ++i) {
        ...
        VkImageView attachments[] = {
            swapchain->views[i],                // swapchain image - color
            swapchain->depth_attachment.view};  // swapchain depth attachment
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time) {
    ...
    // Wait for the current image being available to render
    if (!vulkan_fence_wait(
            &context,
            &context.in_flight_fences[context.current_frame],
            UINT64_MAX)) {
        KWARN("In-flight fence wait failure!");  // not an error but if we start to see too many, we should keep an eye on it,
        return false;
    }
    ...
    // Dynamic state (OPTIONAL) - part of dynamic state
    VkViewport viewport;  // current viewport
    // Vulkan uses different concept but we adopt OPENGL approach (0,0 points) for compatibility
    viewport.x = 0.0f;
    ...
    // Scissor (OPTIONAL) - part of dynamic state
    // What region is gonna be clipped and showed
    VkRect2D scissor;
...
// Should not run if begin failed
...
VKAPI_ATTR VkBool32 VKAPI_CALL vk_debug_callback(
    ...
    return VK_FALSE;  // Always have to return false as specified in the specs.
}
...
b8 recreate_swapchain(renderer_backend* backend) {
    ...
    // Make sure we have most up-to-date format available
    ...
    // Now, generate framebuffers and command buffers
    regenerate_framebuffers(backend, &context.swapchain, &context.main_renderpass);
    create_command_buffers(backend);
