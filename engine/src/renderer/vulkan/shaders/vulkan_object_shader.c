...
b8 vulkan_object_shader_create(vulkan_context* context, vulkan_object_shader* out_shader) {
    ...
    // Scissor
    // Basically size of the screen. Clips out what is out of the screen
    VkRect2D scissor;
    ...
    // Position
    VkFormat formats[1 /*attribute_count*/] = {
        VK_FORMAT_R32G32B32_SFLOAT};  // 32 BITS for R, 32 bits for G, 32 bits for B and its type is float
    u64 sizes[1 /*attribute_count*/] = {
        sizeof(vec3)};
    for (u32 i = 0; i < attribute_count; ++i) {
        attribute_descriptions[i].binding = 0;   // binding index - should match binding desc
        attribute_descriptions[i].location = i;  // attrib location (corresponds to layout (location=0))
