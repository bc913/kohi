...
typedef struct keyboard_state {
    b8 keys[256];  // All defaulted to false (released) (not pressed)
} keyboard_state;
...
void input_system_initialize(u64* memory_requirement, void* state) {
    *memory_requirement = sizeof(input_state);
    ...
    // optional: static variables are already zeroed with initialization
...
b8 input_is_key_up(keys key) {
    if (!state_ptr) {
        return true;  // by default the keys are up
    }
